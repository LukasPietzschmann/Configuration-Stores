...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.45\textwidth}
			\begin{itemize}
				\item \only<1>{File system nodes} \only<2->{\textcolor{gray}{\st{File system nodes}} Namespaces}
	...
	\begin{columns}[c]
		\begin{column}{0.25\textwidth}
			...
			\begin{tikzpicture}[level distance=1cm, sibling distance=1.2cm]
				...
				\draw [arrow, visible on=<6>] (DD) to [bend right=20] (billy-wing);
				\draw [arrow, visible on=<{4-5,7-}>] (billy-wing) to (C);
			\end{tikzpicture}
		\end{column}
