...
\begin{frame}
	\frametitle{History}
	\talknote{2010: Best paper award}
	\talknote{Developement activity declined}
	\begin{columns}[c]
...
\begin{frame}
	...
	\talknote{Whats that fuzz in the beginning}
...
\begin{frame}
	...
	\talknote{Zookeeper does a lot of stuff}
	\talknote{We are mostly interested in the configuration stuff}
...
\begin{frame}
	...
	\talknote{Zookeeper is more a framework to solve challenging aspects of distributed
	computing}
	\centering
...
\begin{frame}
	...
	\talknote{Looks like file system}
	\talknote{Actually namespaces}
	\talknote{Nodes contain: acl, time stamps}
	\talknote<2->{persistent: lifetime of ZooKeeper’s namespace, e.g config data}
	\talknote<2->{ephemeral: deleted when client dies, live check}
	\talknote<2->{sequential: both can be. sequencenumber. used for locking, blocking}
	\talknote<3>{Nodes are not used to hold much data}
	\begin{columns}[c]
		\begin{column}{0.45\textwidth}
...
\begin{frame}
	...
	\talknote<1>{CRUD}
	\talknote<2>{only complete updates (as name implies)}
	\talknote<3>{Sync synchronizes client with leader}
	\begin{columns}[c]
		\begin{column}{0.45\textwidth}
...
\begin{frame}
	\talknote{Mode with one server: testing debugging}
	\talknote{Leader election}
	\contourlength{.09em}
...
\begin{frame}
	...
	\centering
	\talknote{Replicated across all servers}
	\talknote{n/2 + 1 like raft consensus}
	\begin{tikzpicture}
		\begin{scope}
			\node [roundednode] (S1) [] {Server 1};
			\node [roundednode] (S2) [right=of S1] {Server 2};
