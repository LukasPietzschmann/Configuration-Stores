\begin{frame}
	\frametitle{Why not just plain old configuration files?}
	\only<1->{\begin{block}[Reasons]
			\begin{itemize}
				\item<1-> Scenario: Services start \& stop often
					\begin{itemize}
						\item[\triangleright] Hostnames in our configuration have to be changed frequently \& clients have to pull often}
					\end{itemize}
				\item<2-> Configuration files have to be replicated 
					\begin{itemize}
						\item[\triangleright] Risk of staleness
					\end{itemize}
			\end{itemize}
		\end{block}

	\onslide<3->{Solution: Configuration Store with monitoring $\Rightarrow$ changes can be pushed to clients}
\end{frame}

\begin{frame}
	\frametitle{Use Cases}
	\framesubtitle{For Highly Available Distributed Configuration Stores}
	\begin{columns}[c]
		\begin{column}{0.65\textwidth}
			\begin{itemize}
				\item Service Discovery
				\item Configuration of Applications
				\item (Node Coordination)
			\end{itemize}
		\end{column}
		\begin{column}{0.25\textwidth}
			\begin{tikzpicture}
				\duck[longhair, water=blue, think={\tiny Where DB?}, bubblecolour=lightgray]
				\duck[water=blue, body=white!80!brown, bill=white!60!brown, bunny, xshift=90pt, scale=.3, yshift=150pt]
				\duck[water=blue, laughing, xshift=60pt, scale=.3, yshift=100pt]
				\duck[water=blue, mask=red, cape=red, xshift=80pt, scale=.3, yshift=50pt]
